...
TEST(map_for) {
	...
	ASSERT_TRUE(is_type(list, DATA_MAP));
    ASSERT_UEQ(3, map_count(list));
    ASSERT_UEQ(index1, map_get(list, "__1_").max_size);
    ...
    ASSERT_UEQ(3, map_get(list, "3").max_size);
    ASSERT_UEQ(4, map_count(list));

    index4 = 99;
    foreach_map(item in list) {
        if (i == 0) {
            ASSERT_TRUE((iter_type(item) == DATA_LLONG));
            printf("item key is %s, and value is %d\n", indic(item), has(item).integer);
        } else if (i == 1) {
            ASSERT_TRUE((iter_type(item) == DATA_DOUBLE));
            printf("item key is %s, and value is %.1f\n", indic(item), has(item).precision);
        } else if (i == 2) {
            ASSERT_TRUE((iter_type(item) == DATA_CONST_CHAR));
            printf("item key is %s, and value is %d\n", indic(item), has(item).integer);
        } else if (i == 3) {
            ASSERT_TRUE((iter_type(item) == DATA_OBJ));
...
TEST(map_push) {
	...
	ASSERT_TRUE(str_is(value, _value_1));
    ...
    ASSERT_TRUE(str_is(value, _value_3));
    ASSERT_XEQ(map_count(list), 1);
    value = map_pop(list).object;
	ASSERT_TRUE(str_is(value, _value_2));
    ASSERT_XEQ(map_count(list), 0);
